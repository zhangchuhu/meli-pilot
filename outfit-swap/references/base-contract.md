...
The per-run `manifest.json` is a link created by `scripts/task_state.py bind` to the canonical cross-run record state under `~/.codex/state/outfit-swap/tables`. Keep full prompts, immutable attempt history, and stale output tokens there. Ordinary diagnostics may also output full prompts, raw diagnostic arguments, request bodies, raw Base64, and raw data URLs after credential-only redaction through `scripts/payload_security.py`. Never output credentials, API keys, tokens, cookies, signatures, or authorization headers or values; pass record error text through `--error-file`.
