...
- [`scripts/run_lock.py`](scripts/run_lock.py) for the canonical per-user, long-lived same-machine holder/release lifecycle
...
1. Read-only preflight the exact table URL, Python 3.10 or newer, required binaries, `ARK_API_KEY` presence, and active installed `doubao-imagegen` skill. Reject every URL resolution except exactly one table identity as specified in the Base contract. Resolve that skill's current directory from the active skill catalog, read its `SKILL.md` plus required prompting/API references, and locate its bundled `scripts/doubao_imagegen.py`; never hardcode a home-directory path. Stop globally on any pre-lock failure without changing Base or generating images.
2. Start `scripts/run_lock.py hold --base-token '<base-token>' --table-id '<table-id>' --run-id '<run-id>'` in a long-lived process/session. Wait for its ready JSON and keep the process alive across the complete serial run. Only after it is ready, paginate the full Base schema, validate fields/status options, recheck and run `field-create` only for absent `处理明细`, then paginate selected records. Stop globally on any other preflight failure without changing record statuses or generating images.
...
9. Inspect the returned bitmap. On rejection, retain it and call `failure --error-file '<local-error-file>'`, then make one targeted prompt correction; allow no more than three paid edits per target in the current source identity and explicit retry cycle. On restart, a validated active artifact remains the same running attempt and must be re-inspected before any new edit. Because a started request may already have been billed, an absent/incomplete artifact conservatively spends that initiated attempt; it becomes pending below three or terminally failed at three. Never regenerate an accepted target because another target fails.
...
12. In cleanup after normal or handled abnormal exit, use `scripts/run_lock.py release --lock-file '<lock-file>' --run-id '<run-id>'`, then wait for the holder process to exit. External-call failures follow the stop/recovery rules in the QC contract.
